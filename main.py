...
def open_datasets():
    ...
    for s in symbols:
        ...
        if df is not None and len(df) > 50:
            for _, row in df.iterrows():
                open_ = smart_float(row.get("Open", row.get("open", 0)))
                close = smart_float(row.get("Close", row.get("close", 0)))
                high  = smart_float(row.get("High", row.get("high", 0)))
                low   = smart_float(row.get("Low", row.get("low", 0)))
                ds.append({
    ...
    if df2 is not None and len(df2) > 50:
        for _, row in df2.iterrows():
            try:
                open_ = smart_float(row.get("open", 0))
                close = smart_float(row.get("close", 0))
                high  = smart_float(row.get("high", 0))
                low   = smart_float(row.get("low", 0))
                ds.append({
