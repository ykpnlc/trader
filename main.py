...
async def check_active_signals():
    ...
    for s in signals:
        ...
        if (s["direction"] == "LONG" and last_price >= s["tp"]) or (s["direction"] == "SHORT" and last_price <= s["tp"]):
            s["result"] = "win"
            s["close_time"] = datetime.utcnow().isoformat()
            results.append(s)
        # SL
...
def active_signal_exists(exchange, symbol, tf, direction):
    """Aynı sembol-timeframe-direction için aktif signal var mı?"""
    signals = load_signals()
    for s in signals:
        if (
            ...
            for symbol in ex_info["symbols"]:
                for tf in ex_info["timeframes"]:
                    ...
                    try:
                        ...
                        if trend:
                            score += 1
                            direction = "LONG" if trend["type"] == "uptrend" else "SHORT"
                        else:
                            direction = "LONG"
                    except:
                        trend = None
                    ...
                    signal_type_emoji = "🦾SWING" if signal_type == "SWING" else "⚡️SCALP"
                    direction_emoji = "🟢 LONG" if direction == "LONG" else "🔴 SHORT"
                    winrate = update_winrate()

                    # Sadece 9+ ve tekrar etmeyen sinyal gönder
                    if score >= 9 and not active_signal_exists(ex_name, symbol, tf, direction):
                        last_close = candles[-1][4]
                        message = (
                            f"🚨 {direction_emoji} | {ex_name.upper()} | {symbol} | {tf}\n"
                            f"{signal_type_emoji} | 🏅 Score: {score}/12\n"
