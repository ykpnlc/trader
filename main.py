...
def open_datasets():
    ...
    ds = []
    # Yahoo’dan örnek: BTC-USD, ETH-USD, AAPL, EURUSD=X (forex)
    ...
    for s in symbols:
        ...
        if df is not None and len(df) > 50:
            ...
            # trend/direction hesaplamasını her satır için apply ile yapıyoruz.
            df["trend"] = df.apply(lambda row: "uptrend" if row["Close"] > row["Open"] else "downtrend", axis=1)
            df["direction"] = df.apply(lambda row: "LONG" if row["Close"] > row["Open"] else "SHORT", axis=1)
            df["result"] = df.apply(lambda row: "win" if abs(row["Close"]-row["Open"]) > 0.005*row["Open"] else "loss", axis=1)
    ...
    if df2 is not None and len(df2) > 50:
        df2["trend"] = df2.apply(lambda row: "uptrend" if row["close"] > row["open"] else "downtrend", axis=1)
        df2["direction"] = df2.apply(lambda row: "LONG" if row["close"] > row["open"] else "SHORT", axis=1)
        df2["result"] = df2.apply(lambda row: "win" if abs(row["close"]-row["open"]) > 0.005*row["open"] else "loss", axis=1)
