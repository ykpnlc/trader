...
# İlk başlatmada açık kaynaklardan veri çekip dataset’i doldurur (her satır için çalışır!)
def open_datasets():
...
# Gerçek mantıklı örnek modüller (burada gerçekçi teknik analiz ve orderflow örnekleriyle genişlet)
...
def advanced_score(candles, symbol):
    ...
    orderflow_score = np.random.randint(-1,2)  # -1, 0, 1
...
# EXCHANGES (tüm piyasalar için CCXT destekli)
EXCHANGES = {
        ...
        for ex_name, ex_info in MARKETS.items():
            ...
            for symbol in ex_info["symbols"]:
                for tf in ex_info["timeframes"]:
                    ...
                    # Aynı coin aynı yönde başka aktif sinyal varsa tekrar yollama
