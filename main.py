...
# ==================== GERÇEK ANALİZ MODÜLLERİ ====================
def find_liquidity_zones(candles, lookback=30, threshold=0.12):
    highs = [c[2] for c in candles[-lookback:]]
    lows = [c[3] for c in candles[-lookback:]]
    eq_highs = [highs[i] for i in range(1, lookback-1)
                if abs(highs[i]-highs[i-1])/highs[i] < threshold/100]
    eq_lows = [lows[i] for i in range(1, lookback-1)
                if abs(lows[i]-lows[i-1])/lows[i] < threshold/100]
    sweep_up = candles[-1][2] > max(eq_highs) if eq_highs else False
    sweep_down = candles[-1][3] < min(eq_lows) if eq_lows else False
...
def check_bos_choch(candles, lookback=30):
    ...
    bos = (highs[-1] > highest) or (lows[-1] < lowest)
    return bos
...
def breaker_block_liquidity_ema(candles, ema_period=50, lookback=30):
    ...
    if closes[-1] > ema[-1]: return True
    if closes[-1] < ema[-1]: return True
    return False

def detect_candle_pattern(candles, threshold=0.55):
    patterns = []
    for i in range(-10, -1):
        open_, close, high, low = candles[i][1], candles[i][4], candles[i][2], candles[i][3]
        ...
        total = high - low
        if lower_wick > threshold*total and body < (1-threshold)*total:
            patterns.append("bullish_pinbar")
        if upper_wick > threshold*total and body < (1-threshold)*total:
...
def mitigation_rsi_volume(candles, rsi_period=14, lookback=30, vol_mult=1.5):
    ...
    if len(closes) < rsi_period+2: return False
...
def ema_trend(candles, period=20):
    closes = pd.Series([c[4] for c in candles])
    if len(closes) < period+2: return "sideways"
    ema = closes.ewm(span=period).mean()
    if closes.iloc[-1] > ema.iloc[-1]: return "uptrend"
    if closes.iloc[-1] < ema.iloc[-1]: return "downtrend"
    return "sideways"
